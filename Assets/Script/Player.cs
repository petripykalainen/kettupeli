...
public class Player : MonoBehaviour {
    ...
    private void Move()
    {
        ...
        if ( staminaAmount <= 0 && !staminaRegenCalled)
        {
            StartCoroutine(StaminaRegenWait(staminaRegenWaitTime));
            staminaRegenCalled = true;
        }
        if (Input.GetKeyDown(KeyCode.LeftControl) && staminaAmount > 25 && dashReady == true)
        {
            usedSpeed *= dashAmount;
        ...
        if (Input.GetKey("space") && staminaAmount >= staminaDrainAmount)
        {
            usedSpeed *= sprintAmount;
            staminaInUse = true;
        }
        if (Input.GetKey("up"))
        {
        ...
        if (staminaInUse == false && staminaAmount < staminaMaxAmount)
        {
            StartCoroutine(StaminaRegenWait(staminaRegenWaitTime));
            staminaRegenCalled = true;
        }
